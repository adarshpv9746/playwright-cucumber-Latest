...
Then('User should get the products list', async () => {
  ...
  await global.expect(products).to.not.be.empty
})
...
Then(
  ...
  async () => {
    ...
    await global.expect(this.orderId).to.not.be.empty
  }
)
...
Then('Verify the order created in the orders page', async () => {
  ...
  await global.expect(orderStatus).to.be.true
})
...
Then('User should be navigated to Orders page', async function () {
  ...
  await global.expect(text).to.equal('Your Orders')
})
